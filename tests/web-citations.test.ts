...
import {
  ...
  verifyFigures,
} from "../src/pipeline/web-citations";
...
describe("foreign citation markers", () => {
  it("rewrites OpenAI-style markers so the linker can see them", () => {
    ...
    expect(normalizeCitationMarkers("plain 【2】 marker")).toBe(
      "plain [2] marker",
    );
  });
...
describe("sources listed from what was fetched", () => {
  ...
  it("ignores out-of-range read numbers", () => {
    expect(
      appendWebSourcesToMessage("No figures here.", sources, {
        readThisTurn: [0, 99],
      }),
    ).toBe("No figures here.");
  });
});

describe("verifyFigures", () => {
  it("passes an answer with no measured figures", () => {
    expect(verifyFigures("There are 3 renewal options in 2026.", [])).toBe(
      "ok",
    );
  });

  it("flags figures stated when nothing was fetched", () => {
    expect(verifyFigures("It closed at 13.380 TRY, up 2.53%.", [])).toBe(
      "no-evidence",
    );
  });
  ...
  it("passes when a figure appears in the fetched text, including comma decimals", () => {
    expect(
      verifyFigures("It closed at 14.35 TRY.", ["Close 14.35 on Sep 8"]),
    ).toBe("ok");
    expect(verifyFigures("It closed at 14.35 TRY.", ["Kapanis 14,35"])).toBe(
      "ok",
    );
  });
  ...
  it("does not flag a derived value when another figure is grounded", () => {
    expect(
      verifyFigures("Closed at 14.35, up 2.53% from the prior session.", [
        "Close 14.35",
      ]),
    ).toBe("ok");
  });
