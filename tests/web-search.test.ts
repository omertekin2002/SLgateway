...
import type { WebToolsConfig } from "../src/config";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
vi.mock("../src/pipeline/gemini-search", () => ({ searchWeb: vi.fn() }));
import { searchWeb as geminiGroundedSearch } from "../src/pipeline/gemini-search";
import {
  MAX_WEB_SEARCH_RESULTS,
  resolveWebSearchProvider as resolveProvider,
  searchWeb as search,
} from "../src/pipeline/web-search";
...
describe("searchWeb", () => {
  ...
  it("asks Firecrawl for web results only, so search stays a lead list", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValue(
        json({
          success: true,
          data: {
            web: [{ title: "T", url: "https://c.test/", description: "D" }],
          },
        }),
      );
    vi.stubGlobal("fetch", fetchMock);
    ...
    expect(result.results).toEqual([
      { title: "T", url: "https://c.test/", snippet: "D" },
    ]);
  });
  ...
  it("drops duplicates and non-http entries, and caps the result count", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(json({ web: { results } })),
    );
    const result = await searchWeb("q");
    expect(result.results).toHaveLength(MAX_WEB_SEARCH_RESULTS);
    expect(
      result.results.filter((entry) => entry.url === "https://a.test/x"),
    ).toHaveLength(1);
    expect(result.results.some((entry) => entry.url.startsWith("ftp:"))).toBe(
      false,
    );
  });
  ...
  it("wraps provider failures in a public-safe error", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(json({ error: "rate limited" }, 429)),
    );
...
function currentConfig(): WebToolsConfig {
  return {
    provider: process.env.WEB_SEARCH_PROVIDER as WebToolsConfig["provider"],
    braveApiKey: process.env.BRAVE_SEARCH_API_KEY,
    firecrawlApiKey: process.env.FIRECRAWL_API_KEY,
    geminiApiKey: process.env.GEMINI_API_KEY,
  };
}
const resolveWebSearchProvider = () => resolveProvider(currentConfig());
const searchWeb = (query: string) => search(query, { config: currentConfig() });
