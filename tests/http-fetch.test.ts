...
import {
  httpGet,
  MAX_FETCH_RESPONSE_CHARACTERS,
} from "../src/pipeline/http-fetch";
...
it("follows redirects and reports the address that actually answered", async () => {
  ...
  expect(String(fetchMock.mock.calls[1]?.[0])).toBe(
    "https://api.test/v2/quote",
  );
  expect(result.url).toBe("https://api.test/v2/quote");
});

it.each(["http://169.254.169.254/latest/meta-data", "http://localhost.:3000/"])(
  "rejects a redirect to %s",
  async (destination) => {
    const fetchMock = vi.fn().mockResolvedValueOnce(redirect(destination));
    vi.stubGlobal("fetch", fetchMock);
...
it("stops after too many redirects", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn().mockResolvedValue(redirect("https://api.test/loop")),
  );
...
it("truncates oversized bodies", async () => {
  vi.stubGlobal(
    ...
    vi
      .fn()
      .mockResolvedValue(
        new Response("x".repeat(MAX_FETCH_RESPONSE_CHARACTERS + 500)),
      ),
  );
...
it("cancels after the useful prefix without requesting a failed tail", async () => {
  ...
  expect(await httpGet("https://api.test/big")).toMatchObject({
    body: "x".repeat(MAX_FETCH_RESPONSE_CHARACTERS),
    truncated: true,
  });
...
it("preserves multibyte text split across chunks and an exact-length response", async () => {
  ...
  const chunks = [
    bytes.subarray(0, 3),
    bytes.subarray(3, 17003),
    bytes.subarray(17003),
  ];
  vi.stubGlobal(
    "fetch",
...
it("uses the declared charset when decoding and truncating incrementally", async () => {
  ...
  expect(await httpGet("https://api.test/encoded")).toMatchObject({
    body: "€".repeat(MAX_FETCH_RESPONSE_CHARACTERS),
    truncated: true,
  });
});
...
it("wraps transport failures in a public-safe error", async () => {
  vi.stubGlobal(
    "fetch",
    vi.fn().mockRejectedValue(new Error("ECONNREFUSED 1.2.3.4:443")),
  );
